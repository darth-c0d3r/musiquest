...
head -n 500 ../dataset/data_file.txt > temp_top

awk 'BEGIN {FS="\t";} {print $1}' temp_top | sort | uniq > temp
awk 'NR <= 500 {print "insert into artist(name) values ('\''"$0"'\'');"}' temp > insert_data.sql

awk 'BEGIN {FS="\t";} {print $1"\t"$2}' temp_top | sort | uniq > temp
awk 'BEGIN {FS="\t";} NR <= 500 {print "insert into album(name, artist_id) select '\''"$2"'\'', artist_id from artist where artist.name = '\''"$1"'\'';"}' temp >> insert_data.sql
